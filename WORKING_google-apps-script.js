/**
 * Google Apps Script for Trihari Universal Form Submissions - WORKING VERSION
 * Deploy this as a Web App in Google Apps Script
 * 
 * IMPORTANT: This version uses the correct Google Apps Script API
 * 
 * Setup Instructions:
 * 1. Go to script.google.com
 * 2. Open your existing project
 * 3. Replace ALL code with this working version
...
function addSubmissionToSheet(data) {
  try {
    ...
    try {
      ...
      console.log('⚠️ Could not auto-resize columns:', resizeError)
    }
  ...
  try {
    ...
    if (lastRow > 1) {
      try {
        ...
        console.log('⚠️ Could not count today submissions:', dateError)
      }
    }
...
function testSpreadsheetAccess() {
  ...
  try {
    ...
    if (SPREADSHEET_ID === 'YOUR_ACTUAL_SPREADSHEET_ID_HERE') {
      throw new Error('Please update SPREADSHEET_ID with your actual Google Sheet ID')
    }
    ...
    return 'Spreadsheet access test passed!'
    
  } catch (error) {
    console.error('❌ Spreadsheet test failed:', error)
    return 'Spreadsheet test failed: ' + error.toString()
  }
}
